# ⚡ SyntricDB Demo & Test Client Project

This project demonstrates real-time database connectivity and full **CRUD (Create, Read, Update, Delete)** operations against the **SyntricDB AI-Native Database Engine**.
...
```
syntricdb-demo/
├── syntricdb_sdk.py   # Official Python Client SDK for SyntricDB REST API
...
### 1. Ensure SyntricDB Server is Running
```bash
syntricdb status
# If stopped, start it:
syntricdb start
...
```bash
python3 syntricdb-demo/main.py
```
...
```python
from syntricdb_sdk import SyntricDBClient

# Connect to SyntricDB Engine
db = SyntricDBClient("http://localhost:8080")

# 1. Connectivity Check
is_ok, info = db.test_connection()
print(f"Connected to SyntricDB! Active Nodes: {len(info['nodes'])}")
