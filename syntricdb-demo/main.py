#!/usr/bin/env python3
"""
SyntricDB Real-Time Connectivity & CRUD Test Suite
"""
...
def main():
    print("==========================================================================")
    print("⚡ SyntricDB Real-Time Connectivity & CRUD Test Suite ⚡")
    print("==========================================================================")
    
    db = SyntricDBClient("http://localhost:8080")
    
    # 1. TEST CONNECTIVITY
    print("\n📡 Step 1: Testing Connectivity to SyntricDB Engine...")
    is_connected, info = db.test_connection()
    if not is_connected:
        print(f"❌ Connectivity Failed: {info}")
        print("💡 Make sure server is running with: syntricdb start")
        sys.exit(1)
    
    print(f"✅ Connected to SyntricDB Engine successfully!")
