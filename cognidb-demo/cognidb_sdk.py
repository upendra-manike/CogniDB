...
class CogniDBClient:
    """Official CogniDB Client SDK for Python"""
    ...
    def test_connection(self):
        ...
        try:
            req = urllib.request.Request(self.health_endpoint, headers={"User-Agent": "CogniDB-Python-Client/1.0"})
    ...
    def execute_sql(self, sql_query):
        """Execute any SQL / AI / Vector query against CogniDB"""
        payload = json.dumps({"sql": sql_query}).encode('utf-8')
        req = urllib.request.Request(
            self.sql_endpoint,
            data=payload,
            headers={"Content-Type": "application/json", "User-Agent": "CogniDB-Python-Client/1.0"}
        )
        try:
