...
import com.cognidb.engine.snapshot.SnapshotManager;
import com.cognidb.security.SecurityManager;
...
public class SecurityAndRecoveryTest {
    ...
    public void testConnectionStringParsing() {
        String connStr = "cognidb://admin:cognidb_secret_pass@10.0.1.50:8080/production";
        ...
        assertEquals("cognidb_secret_pass", parsed.getPassword());
    ...
    public void testAuthenticationAndRoles() {
        assertTrue(securityManager.validateApiKey("cogni_master_key_99", SecurityManager.Role.ADMIN));
        assertTrue(securityManager.validateApiKey("cogni_rw_key_22", SecurityManager.Role.READ_WRITE));
        assertFalse(securityManager.validateApiKey("cogni_ro_key_11", SecurityManager.Role.ADMIN));
